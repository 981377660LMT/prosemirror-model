...
describe('Node', () => {
  ...
  describe('between', () => {
    function between(doc: Node, ...nodes: string[]) {
      ...
      doc.nodesBetween((doc as any).tag.a, (doc as any).tag.b, (node, pos) => {
        ...
        if (compare != nodes[i++])
          throw new Error(
            'Expected ' + JSON.stringify(nodes[i - 1]) + ', got ' + JSON.stringify(compare)
          )
  ...
  describe('check', () => {
    ...
    it('notices marks in wrong places', () => {
      ist.throws(
        () => doc(schema.nodes.paragraph.create(null, [], [schema.marks.em.create()])).check(),
  ...
  describe('toString', () => {
    it('should have the default toString method [text]', () =>
      ist(schema.text('hello').toString(), '"hello"'))
  ...
  describe('leafText', () => {
    it('should custom the textContent of a leaf node', () => {
      let contact = customSchema.nodes.contact.createChecked({
        name: 'Bob',
        email: 'bob@example.com'
      })
      let paragraph = customSchema.nodes.paragraph.createChecked({}, [
        customSchema.text('Hello '),
        contact
      ])
